...
static void write_to_delta(UINT32 const bank, UINT32 const lpa, UINT32 const buf_addr);
...
static void write_page(UINT32 const lpn, UINT32 const sect_offset, UINT32 const num_sectors)
{
	...
	if (old_vpn != NULL)
	{
		...
		nand_page_read(bank, get_pbn(old_vpn), get_offset(old_vpn), TEMP_BUF_PTR(0));
	...
	else
	{
		if (num_sectors != SECTORS_PER_PAGE)
		{
			if (page_offset != 0)
			{
				mem_set_dram(WR_BUF_PTR(g_ftl_write_buf_id), NULL, page_offset * BYTES_PER_SECTOR);
			}
			// read `right hole sectors'
			if ((page_offset + column_cnt) < SECTORS_PER_PAGE)
			{
...
static void garbage_collection(UINT32 const bank)
{
	uart_printf("gabage collect at %d", bank);
	ASSERT(free_vpn == 0);
	ASSERT(bank < NUM_BANKS);
	...
	for (src_page = 0; src_page < (PAGES_PER_BLK - 1); src_page++)
	{
		...
		if (src_lpn == INVAL)
		{
			...
			nand_page_read(bank, vt_vblock, src_page, FTL_BUF(bank));

			//델타가 몇개 들어있는지 알아냄
			UINT32 delta_cnt = read_dram_32(FTL_BUF(bank));
			UINT32 delta_offset;

			for(delta_offset = 0; delta_offset < delta_cnt; delta_offset++)
			{
				UINT32 lpa = read_dram_32(FTL_BUF(bank) + sizeof(UINT32) * (delta_offset+1) * 2);
				/*
				...
				if(lpa != INVAL)
				{
					...
					else
					{
						...
						if(delta_ppa == vt_vblock*PAGES_PER_BLK + src_page)
						{
							/*
							 * 델타 매핑에 있고
							 * 요놈이 거기 써진 ppa가 맞아!
							...
							UINT32 delta_data_offset = read_dram_32(FTL_BUF(bank) + sizeof(UINT32) * ((delta_offset + 1) * 2 + 1));
	...
	else	//델타 버퍼가 압축된 델타를 못 넣음
	{
		...
		for(i = 0; i < data_cnt; i++)
		{
			...
			if(lpa_out != INVAL)
			{
				...
				set_delta_ppa(offset_out, free_vpn);
			}
		}
		//nand page에 씀!!
		if (free_vpn != 0)
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				set_lpn(bank, new_ppa % PAGES_PER_BLK, old_delta_lpa);
				set_vcount(bank, old_delta_ppa / PAGES_PER_BLK, get_vcount(bank, old_delta_ppa / PAGES_PER_BLK) - 1);
				//set_vcount(bank, get_vcount(bank, get_vpn(lpa) / PAGES_PER_BLK) - 1);
				set_vcount(bank, (new_ppa / PAGES_PER_BLK), (get_vcount(bank, (new_ppa / PAGES_PER_BLK)) + 1));
