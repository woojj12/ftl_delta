...
UINT32 delta_rw_buf_lpn[NUM_DELTA_RW_BUFFERS];
UINT32 delta_rw_buf_pointer;
...
void ftl_read(UINT32 const lba, UINT32 const num_sectors)
{
	...
	while (remain_sects != 0)
	{
		...
		if(debug) uart_printf("read from lpn 0x%x", lpn);
		if(debug) uart_printf("read from vpn 0x%x", vpn);
		if (vpn != NULL)
		{
			//쓴적이 있음
			/* 델타가 있으면
...
static void write_page(UINT32 const lpn, UINT32 const sect_offset, UINT32 const num_sectors)
{
	 ...
	 * 	그 위치에 쓴다
	 */
	UINT32 page_offset = sect_offset;
	UINT32 column_cnt  = num_sectors;
	UINT32 index;
	UINT32 bank, old_vpn, new_vpn;
	UINT32 vblock, page_num;

	for(index = 0; index < NUM_DELTA_RW_BUFFERS; index++)
	{
		if(delta_rw_buf_lpn[index] == lpn)
		{
			goto FOUND_IN_BUFFERS;
		}
	}
	goto NOT_FOUND_IN_BUFFERS;

FOUND_IN_BUFFERS:
	mem_copy(delta_rw_buf_lpn[index] + sect_offset * BYTES_PER_SECTOR, WR_BUF_PTR(g_ftl_write_buf_id) + sect_offset * BYTES_PER_SECTOR, num_sectors * BYTES_PER_SECTOR);
	g_ftl_write_buf_id = (g_ftl_write_buf_id + 1) % NUM_WR_BUFFERS;
	delta_rw_buf_pointer = (index + 1) % NUM_DELTA_RW_BUFFERS;
	return;

NOT_FOUND_IN_BUFFERS:
	...
	if(debug) uart_printf("write_page at lpn 0x%x", lpn);
	CHECK_LPAGE(lpn);
	...
	if (old_vpn != NULL)
	{
		...
		{
			uart_printf("set invalid original, vcount %d", get_vcount(bank, vblock));
	...
	if(debug) uart_printf("write to bank %d vpn 0x%x", bank, new_vpn);
	...
	//invalid delta map
	UINT32 delta_map_offset = get_delta_map_offset(lpn);
	if(delta_map_offset != INVAL)
	{
		set_delta_lpa(delta_map_offset, 0);
...
static UINT32 assign_new_write_vpn(UINT32 const bank)
{
	...
	{
		uart_printf("assign new write, vcount %d", get_vcount(bank, vblock));
		...
		uart_printf("free blk count bank %d %d", bank, g_misc_meta[bank].free_blk_cnt);
	...
	if(debug) uart_printf("assign vpn 0x%x", write_vpn);
	return write_vpn;
}
...
static void garbage_collection(UINT32 const bank)
{
	debug = 1;
	gc_bank = bank;
	uart_printf("garbage collect at %d", bank);
	free_vpn = 0;
	ASSERT(free_vpn == 0);
	ASSERT(bank < NUM_BANKS);
	...
	uart_printf("vcount before gc %d", vcount);
	...
	for (src_page = 0; src_page < (PAGES_PER_BLK - 1); src_page++)
	{
		...
		{
			/*
			 * if the page is delta page
			 * read the page
			 * for each deltas,
			 *
			 */
			uart_printf("delta page");
			UINT32 delta_offset;
			nand_page_read(bank, vt_vblock, src_page, COPY_BUF(bank));
			UINT32 delta_cnt = read_dram_32(COPY_BUF(bank));

			uart_printf("delta page, delta cnt %d", delta_cnt);

			for(delta_offset = 0; delta_offset < delta_cnt; delta_offset++)
			{
				UINT32 delta_lpa = read_dram_32(COPY_BUF(bank) + sizeof(UINT32) * (delta_offset+1) * 2);
				if(delta_lpa == INVAL)
				{
					//invalid
					uart_printf("invalid delta");
					continue;
				}
				else
				{
					UINT32 delta_map_offset = get_delta_map_offset(delta_lpa);
					if(delta_map_offset == INVAL)
					{
						//쓰고 나서 델타가 바뀐 경우
						uart_printf("invalid delta");
					...
					else if(get_delta_ppa(delta_map_offset) == src_page + vt_vblock * PAGES_PER_BLK)
					{
						//쓰고 나서 델타가 바뀐 경우
						uart_printf("invalid delta");
						continue;
					...
					{
						uart_printf("valid delta, delta_lpa %d", delta_lpa);
						set_delta_ppa(delta_map_offset, 0);
						UINT32 delta_data_offset = read_dram_32(COPY_BUF(bank) + sizeof(UINT32) * ((delta_offset + 1) * 2 + 1));
						write_to_delta(bank, delta_lpa, delta_data_offset);
					}
				}
			}
		}
		else
		{
			 ...
			 * data page
			 */
			//uart_printf("data page");
			UINT32 _old_vpn = get_vpn(src_lpn);
			UINT32 old_vpn = set_valid_ppa(_old_vpn);
			if (old_vpn != ((vt_vblock * PAGES_PER_BLK) + src_page))
			{
				//invalid data page
				continue;
			}
			else
			{
				if(vcount == 0)
				{
					uart_printf("vcount 0, print");
					uart_printf("src lpn %d", src_lpn);
					uart_printf("old_vpn %d", old_vpn);
					uart_printf("_old_vpn %d", _old_vpn);
				}
				...
				//set has delta or not
				set_vpn(src_lpn, (free_vpn | (_old_vpn & 0x8000)));
	...
	if (vcount == 0)
	{
		uart_printf("free_vpn %d", free_vpn);
		ASSERT(free_vpn == (gc_vblock * PAGES_PER_BLK));
	}
#endif
	// 3. erase victim block
	nand_block_erase(bank, vt_vblock);
	uart_printf("free vpn %d, mod %d", free_vpn, free_vpn % PAGES_PER_BLK);
...
static UINT32 get_vt_vblock(UINT32 const bank)
{
	...
	ASSERT(vblock >= META_BLKS_PER_BANK && vblock < VBLKS_PER_BANK);
	uart_printf("vt block vcount %d", get_vcount(bank, vblock));
	ASSERT(get_vcount(bank, vblock) < (PAGES_PER_BLK - 1));
...
static void format(void)
{
	...
	uart_printf("NUM_DELTA_RW_BUFFERS %d", NUM_DELTA_RW_BUFFERS);
	uart_printf("NUM_RW_BUFFERS %d", NUM_RW_BUFFERS);
	uart_printf("NUM_RD_BUFFERS %d", NUM_RD_BUFFERS);
	uart_printf("NUM_WR_BUFFERS %d", NUM_WR_BUFFERS);
...
static void init_metadata_sram(void)
{
	...
	for(index = 0; index < NUM_DELTA_RW_BUFFERS; index++)
	{
		delta_rw_buf_lpn[index] = INVALID;
	}
	delta_rw_buf_pointer = 0;
	...
	for (bank = 0; bank < NUM_BANKS; bank++)
	{
		...
		{
			vblock++;
			// �꾩옱 next vblock遺�꽣 �덈줈���곗씠�곕� ��옣���쒖옉
...
static BOOL32 compress(UINT32 buf_data, UINT32 buf_write)
{
	...
	if(debug) uart_printf("xor_buffer");
	...
	if(debug) uart_printf("lzf_compress : %d", success);
	...
	else	//델타 버퍼가 압축된 델타를 못 넣음
	{
		if(free_vpn != 0)
		{
			...
			nand_page_program(bank,
					...
					free_vpn % PAGES_PER_BLK,
					DELTA_BUF(bank));
			set_lpn(bank, (free_vpn % PAGES_PER_BLK), INVAL);
			uart_printf("write to delta free vpn %d", free_vpn);
			free_vpn++;
		}
		else
		{
		...
		{
			//none gc
			UINT32 new_vpn = assign_new_write_vpn(bank);
			nand_page_program(bank,
					new_vpn / PAGES_PER_BLK,
					new_vpn % PAGES_PER_BLK,
					DELTA_BUF(bank));
			set_lpn(bank, (new_vpn % PAGES_PER_BLK), INVAL);
		}
